												...
												<td>
													{if $option.unique_carrier}
														{foreach $option.carrier_list as $carrier}
															...
															{* InPost.sn *}
															{if $carrier.instance->name == 'InPost'}
																<input type="hidden" name="inpost_key" id="inpost_key" value='{$key}'>
																<br />{$inpost_map} | {$short_locker_select} {$locker_select}
																{$show_all_terminals}
																<br />{$inpost_mobile}
															{/if}
															{* InPost.en *}
...
{if $conditions}
{* InPost.sn *}
	{addJsDefL name=msg_inpost_check_extra_fields}{l s='You must select a machine and fill the mobile number.' js=1}{/addJsDefL}
{* InPost.en *}
